...
if __name__ == '__main__':
    ...
    plot_models_over_memory = False
    plot_heatmaps = False

    csv_file = open("./data/model_comparisons/analysis_avg_solution_time_heterogeneous.csv", "r")
    ...
    for heterogeneity_model_option in heterogeneity_model_options:
        for probability_matching_option in probability_matching_options:
            ...
            plt.gca().set_prop_cycle(plt.cycler('color', my_cmap(np.linspace(0, 1, 11))))

            plt.title("%s with %s: [0.00, %.2f]" % (probability_matching_option, heterogeneity_model_option[0], heterogeneity_model_option[1]))
            plt.xlabel("rewiring probability, q")
            ...
            if probability_matching_option == "deterministic":
                plt.legend(loc="upper right")
            plt.savefig("./data/model_comparisons/comparison_plots_heterogeneous/comparison-%s-with-%s_0.00_%.2f.png" % (probability_matching_option, heterogeneity_model_option[0], heterogeneity_model_option[1]))
            print("file saved")
            plt.close() 
    ...
    if plot_individual_models:
        ...
        for model_option in model_options:
            ...
            # plt.title("%s, Memory: %d" % (model_option[0], model_option[1]))
            # plt.xlabel("rewiring probability, q")
            # plt.ylabel("running time (iterations), max 180")
            ...
            # plt.plot(q_options, model_avg_solution_time, "o-", label=(("%s-memory_%d")%(model_option[0], model_option[1])))
            # plt.legend()
            # plt.savefig("./data/model_comparisons/comparison_plots/comparison-%s-memory_%d.png" % (model_option[0], model_option[1]))
            # plt.close()
    ...
    if plot_models_over_memory:
        ...
        for probability_matching_option in probability_matching_options:
            ...
            plt.title("%s" % (probability_matching_option))
            plt.xlabel("rewiring probability, q")
            plt.ylabel("running time (iterations), max 180")
            ...
            for memory_option in memory_options:
                ...
                if (i-1) % 30 == 0:
                    plt.plot(q_options, model_avg_solution_time, "o-", label=(("%s-memory_%d")%(probability_matching_option, memory_option)))
            ...
            plt.savefig("./data/model_comparisons/comparison_plots/comparison-%s.png" % (probability_matching_option))
            plt.close()
