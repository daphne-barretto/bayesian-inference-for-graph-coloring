...
def run_set_of_trials_across_q(args, trials=0):
    ...
    for q in q_list:
        ...
        for unstuckness_high in unstuckness_high_list:
            args.stubbornness_high = unstuckness_high
            # model_location = args.model_location
            model_location = "./data/individual_models_with_stubbornness_0.00_%.2f" % unstuckness_high
            args.model_location = "./data/individual_models_with_stubbornness_0.00_%.2f" % unstuckness_high
            model_option_location = model_location + "/%s-memory_%d-stubbornness_0.00_%.2f-randomness_0.00_0.00-unstuckness_0.00_0.00" % ("probability_matching", args.memory, unstuckness_high)
        ...
        for unstuckness_high in unstuckness_high_list:
            ...
            model_location = "./data/individual_models_with_stubbornness_0.00_%.2f" % unstuckness_high
            args.model_location = "./data/individual_models_with_stubbornness_0.00_%.2f" % unstuckness_high
            model_option_location = model_location + "/%s-memory_%d-stubbornness_0.00_%.2f-randomness_0.00_0.00-unstuckness_0.00_0.00" % ("deterministic", args.memory, unstuckness_high)
            trial_location = model_option_location + "/max_iterations_180-cliques_6-nodes_per_clique_6-colors_9-q_%.2f" % q
            if not os.path.exists(trial_location):
                os.makedirs(trial_location + "/animation/")
                os.makedirs(trial_location + "/csv/")
