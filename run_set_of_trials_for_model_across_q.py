...
def run_set_of_trials_across_q(args, trials=0):
    ...
    for q in q_list:
        ...
        model_option_location = model_location + "/%s-memory_%d-stubbornness_0.00_0.00-randomness_0.00_0.00-unstuckness_0.00_1.00" % ("probability_matching", args.memory)
        ...
        while len(os.listdir(trial_location + "/csv/")) < 100:
            args.probability_matching = True
            run_trial(args)

        model_location = args.model_location
        model_option_location = model_location + "/%s-memory_%d-stubbornness_0.00_0.00-randomness_0.00_0.00-unstuckness_0.00_1.00" % ("deterministic", args.memory)
        trial_location = model_option_location + "/max_iterations_180-cliques_6-nodes_per_clique_6-colors_9-q_%.2f" % q
        if not os.path.exists(trial_location):
            os.makedirs(trial_location + "/animation/")
            os.makedirs(trial_location + "/csv/")
